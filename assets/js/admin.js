...
(function () {
  ...
  function bindTabs() {
    $$('.tab').forEach(function (t) {
      t.addEventListener('click', function () {
        ...
        if (name === 'reglages') chargerReglagesForm();
    ...
    $('#btn-save-reglages').addEventListener('click', sauverReglages);
    $('#btn-dispos').addEventListener('click', modalDispos);
    $('#view-jour').addEventListener('click', function () { planningMode = 'jour'; planningRef = C.todayISO(); chargerPlanning(); });
    $('#view-semaine').addEventListener('click', function () { planningMode = 'semaine'; chargerPlanning(); });
  }
  ...
  function modalReplanifier(r) {
    ...
    async function render() {
      ...
      const [horaires, exceptions, resList] = await Promise.all([
        ...
        sb.from('reservations').select('id,heure_debut,heure_fin,statut').eq('date_rdv', dateStr).is('supprime_le', null).neq('id', r.id)
      ]);
  ...
  function modalPrestation(p) {
    ...
    body.innerHTML =
      ...
      '<div style="display:flex;gap:10px">' +
        ...
        '<div style="flex:1"><label class="field">Duree de base (min)</label><input type="number" id="p-duree" value="' + est.duree_base_min + '" /></div>' +
      '</div>' +
      ...
      '<label class="field">Ordre d\'affichage</label><input type="number" id="p-ordre" value="' + est.ordre + '" />' +
    ...
    $('#p-save', body).addEventListener('click', async function () {
      const patch = {
        ...
        ordre: Number($('#p-ordre', body).value) || 0
      };
  ...
  // ================= REGLAGES =================
  async function chargerReglagesForm() {
  ...
  async function sauverReglages() {
    const inputs = $$('#reglages-form input');
    const patchByCle = {};
  ...
  // ---------- Modale disponibilites ----------
  async function modalDispos() {
    const [horaires, exceptions] = await Promise.all([C.getHoraires(), C.getExceptions()]);
    const body = document.createElement('div');
    let h = '<h2>Mes disponibilites</h2><h3>Horaires par defaut</h3>';
    for (let j = 0; j < 7; j++) {
      const hr = horaires.find(function (x) { return x.jour_semaine === j; }) || { ouvert: false, heure_debut: '10:00', heure_fin: '18:00' };
      h += '<div style="display:flex;align-items:center;gap:8px;padding:6px 0;border-bottom:1px solid var(--gris-rose)">' +
        '<label style="width:90px;display:flex;align-items:center;gap:6px;font-weight:500"><input type="checkbox" data-jour="' + j + '" class="d-ouvert" ' + (hr.ouvert ? 'checked' : '') + ' style="width:18px;height:18px"/> ' + C.JOURS[j].slice(0, 3) + '</label>' +
        '<input type="time" class="d-debut" data-jour="' + j + '" value="' + (hr.heure_debut ? hr.heure_debut.slice(0, 5) : '10:00') + '" style="flex:1" />' +
        '<input type="time" class="d-fin" data-jour="' + j + '" value="' + (hr.heure_fin ? hr.heure_fin.slice(0, 5) : '18:00') + '" style="flex:1" />' +
        '</div>';
    }
    h += '<button class="btn btn-block mt" id="d-save">Enregistrer les horaires</button>';
    h += '<h3 style="margin-top:22px">Exceptions (fermeture ou horaire special)</h3><div id="exc-list"></div>';
    h += '<label class="field">Ajouter une exception</label>' +
      '<input type="date" id="exc-date" min="' + C.todayISO() + '" />' +
      '<label style="display:flex;align-items:center;gap:8px;font-weight:400;margin-top:8px"><input type="checkbox" id="exc-ouvert" style="width:18px;height:18px"/> Ouvert (sinon ferme ce jour)</label>' +
      '<div style="display:flex;gap:8px;margin-top:8px"><input type="time" id="exc-debut" value="10:00" style="flex:1"/><input type="time" id="exc-fin" value="18:00" style="flex:1"/></div>' +
      '<button class="btn btn-ghost btn-block mt" id="exc-add">Ajouter l\'exception</button>';
    body.innerHTML = h;
    const modal = ouvrirModal(body);

    function renderExc() {
      const zone = $('#exc-list', body);
      if (!exceptions.length) { zone.innerHTML = '<p class="hint">Aucune exception.</p>'; return; }
      zone.innerHTML = exceptions.map(function (e) {
        return '<div class="recap-line"><span>' + C.formatDateFR(e.date) + '</span><span>' +
          (e.ouvert ? (e.heure_debut ? e.heure_debut.slice(0, 5) + '-' + e.heure_fin.slice(0, 5) : 'Ouvert') : 'Ferme') +
          ' <button class="btn btn-sm btn-danger" data-del="' + e.id + '">x</button></span></div>';
      }).join('');
      $$('[data-del]', zone).forEach(function (b) {
        b.addEventListener('click', async function () {
          await sb.from('exceptions_dispo').delete().eq('id', this.dataset.del);
          const idx = exceptions.findIndex(function (x) { return x.id === b.dataset.del; });
          if (idx >= 0) exceptions.splice(idx, 1);
          renderExc(); toast('Exception supprimee.');
        });
      });
    }
    renderExc();

    $('#d-save', body).addEventListener('click', async function () {
      for (let j = 0; j < 7; j++) {
        const ouvert = $('.d-ouvert[data-jour="' + j + '"]', body).checked;
        const debut = $('.d-debut[data-jour="' + j + '"]', body).value;
        const fin = $('.d-fin[data-jour="' + j + '"]', body).value;
        await sb.from('horaires_par_defaut').update({ ouvert: ouvert, heure_debut: debut, heure_fin: fin }).eq('jour_semaine', j);
      }
      toast('Horaires enregistres.');
    });

    $('#exc-add', body).addEventListener('click', async function () {
      const date = $('#exc-date', body).value;
      if (!date) { toast('Choisissez une date.'); return; }
      const ouvert = $('#exc-ouvert', body).checked;
      const row = { date: date, ouvert: ouvert, heure_debut: ouvert ? $('#exc-debut', body).value : null, heure_fin: ouvert ? $('#exc-fin', body).value : null };
      const { data, error } = await sb.from('exceptions_dispo').upsert(row, { onConflict: 'date' }).select().single();
      if (error) { console.error(error); toast('Erreur.'); return; }
      const idx = exceptions.findIndex(function (x) { return x.date === date; });
      if (idx >= 0) exceptions[idx] = data; else exceptions.push(data);
      renderExc(); toast('Exception ajoutee.');
    });
  }
  ...
  $('#btn-logout').addEventListener('click', function () { try { sessionStorage.removeItem('faith_admin'); } catch (e) {} location.reload(); });
